...
(function() {
	...
	ixmaps.legend.makeColorLegendHTML = function(szId,szLegendId){
		...
		for ( var i=0; i<max; i++){
			if(  themeObj.szFlag.match(/SUM/) || 
				...
				if ( typeof(themeObj.partsA[i].nSum) != "undefined" ){
					sortA.push({index:i,count:(themeObj.partsA[i].nSum)});
				}else{
		...
		if ( fColorScheme &&
			...
			for ( var i=0; i<max; i++){
				...
				var szCount = sortA[i].count?("<td class='theme-legend-count' >"+count+"&nbsp;"+(themeObj.szLegendUnits||themeObj.szUnits||"").replace(/ /g,'&nbsp;')+"</td>"):"";
	 ...
	 */
	ixmaps.htmlgui_onDrawThemexx = function(szId){ 
	 ...
	 * @param szId the id of the theme
	 */
	ixmaps.htmlgui_onRemoveThemexx = function(szId){
