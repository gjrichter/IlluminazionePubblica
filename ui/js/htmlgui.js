...
(function( ixmaps, $, undefined ) {
	 ...
	 * @type void
	 */
	ixmaps.HTML_showLoading = function(szMessage){
	...
	ixmaps.htmlgui_onMapInit = function(mapwindow){
		...
		$("#loading-text").empty();
		$("#loading-text").append("... initializing ...");
	...
	ixmaps.htmlgui_onMapReady = function(mapwindow){
		...
		**/
		...
		if ( 1 || !ixmaps.window.location || !(String(ixmaps.window.location).match(/newMapTheme/)) ){
		...
		// do initial HTML map resize 
		// do it with timeout, so we can include bookmarks programmed by user defined .onMapReady()
		setTimeout("ixmaps.htmlgui_synchronizeMap(false,true)",1);
...
//			htmlgui_setCookie('mapenvelope', szEnvelope);
	...
	ixmaps.htmlgui_onInfoTitle = function(szText){
		return ixmaps.parentApi.htmlgui_onInfoTitle(szText);
	};
