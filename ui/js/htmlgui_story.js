	...
	ixmaps.loadStory = function(szUrl,szFlag,target){
		...
		**/
		...
		if ( szUrl && typeof(szUrl) == "string" && szUrl.length ){
			...
			//$("head").append($("<link rel='stylesheet' href='"+ szStoryRoot+szUrl.split('.')[0]+"_fonts.css"+ "' type='text/css' media='screen' />"));
			//$("head").append($("<link rel='stylesheet' href='"+ szStoryRoot+szUrl.split('.')[0]+".css"+ "' type='text/css' media='screen' />"));

			alert(szStoryRoot+szUrl.split('.')[0]+".css");
...
//		ixmaps.changeThemeStyle(szThemeId,'type:FILTER','add');	
	...
	ixmaps.filterThemeItems = function(szThemeId,szFilter,mode){
		...
		if ( themeObj && themeObj.szFlag.match(/AGGREGATE/) ){
	 ...
	 * @return void
	 */
	ixmaps.showLoading = function(szMessage){
		ixmaps.embeddedApi.showLoading(szMessage);
	};
	 ...
	 * @return void
	 */
	ixmaps.setBoundsXXX = function(bounds){
		var embeddedSVG = ixmaps.embeddedSVG || ixmaps.embeddedApi.embeddedSVG;
		embeddedSVG.window.map.Api.doSetMapToGeoBounds(
			bounds[0],
			bounds[1],
			bounds[2],
			bounds[3]);
	 ...
	 * @return void
	 */
	ixmaps.setView = ixmaps.setView || function(center,nZoom){
		this.dispatchToembeddedApi("map","setView",[center,nZoom]);
	};
