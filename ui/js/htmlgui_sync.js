...
* ------------------------------------------------------------------ */
	...
	/** 2nd possibility
	 *  if click on map (not move!), then switch to SVG
	...
	**/
	...
	ixmaps.do_mapclick = function(e){ return;
		if ( !ixmaps.mouseDownX || (
			 (ixmaps.mouseDownX == ixmaps.mouseMoveX) && 
			 (ixmaps.mouseDownY == ixmaps.mouseMoveY)  )
			){
			var now = new Date();
			if ( !this.isTouchDevice() || ((now.getTime() - ixmaps.lastMoveTime) > 500) ){
				__switchInputMode();
				ixmaps.fSwitchInputModeOnMapClick = true;
			}
		}
...
**/
	...
	function __doSynchronizeSVGMap(fPanOnly) {
		...
		***/
		...
		if ( fSyncMultiMaps && ixmaps.fSync && (ixmaps.fSync == true) && !ixmaps.fSyncBySVG && !ixmaps.fSyncByMasterMap ){
			try{
...
* ------------------------------------------------------------------ */
	...
	**/
	htmlgui_panSVG = function() {
		...
		if ( fSyncMultiMaps && ixmaps.fSync && (ixmaps.fSync == true) && !ixmaps.fSyncBySVG ){
			try{
				var ptLatLon = htmlMap_getCenter();
				ixmaps.syncSlaveMap(ptLatLon,
									ptLatLon,htmlMap_getZoom());
...
* ------------------------------------------------------------------ */
	...
	ixmaps.changeThemeStyle = function(szTheme,szStyle,szFlag){
		try { ixmaps.embeddedSVG.window.map.Api.changeThemeStyle(szTheme,szStyle,szFlag);
		} catch (e){}
	};
	ixmaps.getThemeStyleString = function(szTheme){
		try { return ixmaps.embeddedSVG.window.map.Api.getMapThemeStyleString(szTheme);
		} catch (e){}
	};
...
//			fDragSVGHidden = true;
	...
	ixmaps.htmlgui_setCurrentCenterByGeoBounds = function(ptCenter){
		...
		if ( ixmaps.htmlMap && !ixmaps.fInSVGSync ){
			...
			// try to sync slave maps
			if ( fSyncMultiMaps && ixmaps.fSync && (ixmaps.fSync == true) ){
				try{
					var arrayPtLatLon = htmlMap_getBounds();
